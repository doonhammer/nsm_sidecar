...
import (
	...
	"github.com/nsm_sidecar/pkg/nsm/apis/sidecardataplane"
...
// RequestDeleteConnect implements method for sidecardataplane proto
func (d DataplaneController) RequestDeleteConnect(ctx context.Context, in *sidecardataplane.DeleteConnectRequest) (*sidecardataplane.DeleteConnectReply, error) {

	podName := in.Pod.Metadata.Name
	if podName == "" {
		logrus.Error("sidecar-dataplane: missing required pod name")
		return &sidecardataplane.DeleteConnectReply{
			Deleted:     false,
	...
	switch in.PodType {
	case sidecardataplane.NSMPodType_NSE:
	case sidecardataplane.NSMPodType_NSMCLIENT:
	default:
		logrus.Error("sidecar-dataplane: invalid pod type")
		return &sidecardataplane.DeleteConnectReply{
			Deleted:     false,
...
// RequestBuildConnect implements method for sidecardataplane proto
func (d DataplaneController) RequestBuildConnect(ctx context.Context, in *sidecardataplane.BuildConnectRequest) (*sidecardataplane.BuildConnectReply, error) {

	podName1 := in.SourcePod.Metadata.Name
	if podName1 == "" {
		logrus.Error("sidecar-dataplane: missing required pod name")
		return &sidecardataplane.BuildConnectReply{
			Built:      false,
	...
	logrus.Infof("sidecar-dataplane: attempting to interconnect pods %s/%s and %s/%s",
		podNamespace1,
		podName1,
		podNamespace2,
		podName2)
	...
	if err := connectPods(d.k8s, podName1, podName2, podNamespace1, podNamespace2); err != nil {
		logrus.Errorf("sidecar-dataplane: failed to interconnect pods %s/%s and %s/%s with error: %+v",
			podNamespace1,
			podName1,
			podNamespace2,
			podName2,
		...
		return &sidecardataplane.BuildConnectReply{
			Built: false,
			BuildError: fmt.Sprintf("sidecar-dataplane: failed to interconnect pods %s/%s and %s/%s with error: %+v",
				podNamespace1,
				podName1,
				podNamespace2,
				podName2,
	...
	if err := finalizerutils.AddPodFinalizer(d.k8s, podName1, podNamespace1); err != nil {
		logrus.Errorf("sidecar-dataplane: failed to add finalizer to pod %s/%s with error: %+v", podNamespace1, podName1, err)
	}
	if err := finalizerutils.AddPodFinalizer(d.k8s, podName2, podNamespace2); err != nil {
		logrus.Errorf("sidecar-dataplane: failed to add finalizer to pod %s/%s with error: %+v", podNamespace2, podName2, err)
	}
...
func getContainerID(k8s *kubernetes.Clientset, pn, ns string) (string, error) {
	...
	for _, p := range pl.Items {
		if strings.HasPrefix(p.ObjectMeta.Name, pn) {
			...
			return "", fmt.Errorf("sidecar-dataplane: none of containers of pod %s/%s is in running state", p.ObjectMeta.Namespace, p.ObjectMeta.Name)
		}
	}

	return "", fmt.Errorf("sidecar-dataplane: pod %s/%s not found", ns, pn)
}
...
func main() {
	...
	if err := tools.SocketCleanup(socket); err != nil {
		logrus.Fatalf("sidecar-dataplane: failure to cleanup stale socket %s with error: %+v", socket, err)
	}
	dataplaneConn, err := net.Listen("unix", socket)
	if err != nil {
		logrus.Fatalf("sidecar-dataplane: fail to open socket %s with error: %+v", socket, err)
	}
	dataplaneController.dataplaneServer = grpc.NewServer()
	sidecardataplane.RegisterBuildConnectServer(dataplaneController.dataplaneServer, dataplaneController)

	go func() {
		wg.Add(1)
		if err := dataplaneController.dataplaneServer.Serve(dataplaneConn); err != nil {
			logrus.Fatalf("sidecar-dataplane: failed to start grpc server on socket %s with error: %+v ", socket, err)
		}
	}()
	// Check if the socket of device plugin server is operation
	testSocket, err := tools.SocketOperationCheck(socket)
	if err != nil {
		logrus.Fatalf("sidecar-dataplane: failure to communicate with the socket %s with error: %+v", socket, err)
	}
	testSocket.Close()

	logrus.Infof("sidecar-dataplane: sidecar Dataplane controller is ready to serve...")
...
func deleteLink(k8s *kubernetes.Clientset, podName string, namespace string, podType sidecardataplane.NSMPodType) error {
